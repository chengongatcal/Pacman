...
def joinFactors(factors):
    ...
    joined = factors[0].variableDomainsDict()
    joinedFactor = Factor(unconditionedSet, conditionedSet, joined)

    #print joinedFactor

    #print conditionedSet

    #print unconditionedSet
    assignments = joinedFactor.getAllPossibleAssignmentDicts()
    for assignment in assignments:
        joinedFactor.setProbability(assignment, 1.0)

        for f in factors:
            probFactor = joinedFactor.getProbability(assignment)
            probF = f.getProbability(assignment)
