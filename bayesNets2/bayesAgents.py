...
def constructBayesNet(gameState):
    ...
    for house in gameState.getPossibleHouses():
        for walls in gameState.getHouseWalls(house):
            ...
            obs = OBS_VAR_TEMPLATE % walls
            edges.append((FOOD_HOUSE_VAR, obs))
            edges.append((GHOST_HOUSE_VAR, obs))
            variableDomainsDict[OBS_VAR_TEMPLATE % walls] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]
    ...
    #add positions and values into variableDomainsDict
...
def fillYCPT(bayesNet, gameState):
    ...
    #import probability
    top = {Y_POS_VAR: BOTH_TOP_VAL}
    bottom = {Y_POS_VAR: BOTH_BOTTOM_VAL}
    leftTop = {Y_POS_VAR: LEFT_TOP_VAL}
    leftBottom = {Y_POS_VAR: LEFT_BOTTOM_VAL}

    yFactor.setProbability(top, PROB_BOTH_TOP)
    yFactor.setProbability(leftTop , PROB_ONLY_LEFT_TOP)

    yFactor.setProbability(bottom, PROB_BOTH_BOTTOM)
    yFactor.setProbability(leftBottom, PROB_ONLY_LEFT_BOTTOM)
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    # util.raiseNotDefined()
    def posCheck(wallp, housep):
        check0 = abs(wallp[0] - housep[0])
        check1 = abs(wallp[1] - housep[1])
        return (check0 <= 1) and (check1 <= 1)

    for house in gameState.getPossibleHouses():
        for walls in gameState.getHouseWalls(house):
            obs = OBS_VAR_TEMPLATE % walls
            obsFactor = bn.Factor([obs], [GHOST_HOUSE_VAR, FOOD_HOUSE_VAR], bayesNet.variableDomainsDict())

            if posCheck(walls, bottomLeftPos):
                pos = BOTTOM_LEFT_VAL
            elif posCheck(walls, bottomRightPos):
                pos = BOTTOM_RIGHT_VAL
            elif posCheck(walls, topLeftPos):
                pos = TOP_LEFT_VAL
            elif posCheck(walls, topRightPos):
                pos = TOP_RIGHT_VAL
            else:
                pos = None
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                if assignment[FOOD_HOUSE_VAR] == pos:
                    if assignment[obs] == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif assignment[obs] == BLUE_OBS_VAL:
                        prob = 1 - PROB_FOOD_RED
                    else:
                        prob = 0
               
                elif assignment[GHOST_HOUSE_VAR] == pos:
                    if assignment[obs] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assignment[obs] == BLUE_OBS_VAL:
                        ...
                        prob = 0
                #print assignment[obs]
                else:
                    if assignment[obs] == NO_OBS_VAL:
                        prob = 1
                    else:
                        prob = 0
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    "*** YOUR CODE HERE ***"
    # util.raiseNotDefined()
    p = ['foodHouse', 'ghostHouse']
    factor = inference.inferenceByVariableElimination(bayesNet, p, evidence, eliminationOrder)
    prob = 0.0
    assign = dict()

    assignments = factor.getAllPossibleAssignmentDicts()
    for assignment in assignments:
        curProb = factor.getProbability(assignment)

        if prob < curProb:
            prob = max(prob, curProb)
            assign = assignment
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        variable = [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
        var = inference.inferenceByVariableElimination(self.bayesNet, variable, evidence, eliminationOrder)
        
        left = {}
        right = {}
        assignments = var.getAllPossibleAssignmentDicts()
        ...
        leftProb = var.getProbability(left)
        rightProb = var.getProbability(right)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        for prob, e in self.getExplorationProbsAndOutcomes(evidence):
            vals = self.computeEnterValues(e, enterEliminationOrder)
            var = max(vals)
            expectedValue += prob * var
