...
def enhancedFeatureExtractor(datum):
    ...
    def featureHelper(height, width, feature):
    	if height < DIGIT_DATUM_HEIGHT -1:
            length_0 = DIGIT_DATUM_HEIGHT*width+height+1
            if feature[length_0] == 0:
                feature[length_0] = 1
                featureHelper(height+1, width, feature)
        if height >= 1:
            length_1 = DIGIT_DATUM_HEIGHT*width+height-1
            if feature[length_1] == 0:
                feature[length_1] = 1
                featureHelper(height-1, width, feature)
        if width < DIGIT_DATUM_WIDTH -1:
            length_2 = DIGIT_DATUM_HEIGHT*(width+1)+height
            if feature[length_2] == 0:
                feature[length_2] = 1
                featureHelper(height,width+1, feature)
        if width >= 1:
            length_3 = DIGIT_DATUM_HEIGHT*(width-1)+height
            if feature[length_3] == 0:
                feature[length_3] = 1
                featureHelper(height, width-1, feature)
    ...
    for height in range(DIGIT_DATUM_HEIGHT):
        for width in range(DIGIT_DATUM_WIDTH):
            length = height+width*DIGIT_DATUM_HEIGHT
            check = feature[length]
            if check == 0:
                # count++
                count+=1
                featureHelper(height, width, feature)
    
    update_feature = np.zeros(3, int)
    if count==1 or count == 2 or count == 3:
        index = count - 1
        update_feature[index] = count
    ret = np.append(features, update_feature)
    return ret
