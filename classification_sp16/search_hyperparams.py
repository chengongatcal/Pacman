...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        ...
        minibatchSGDsolver = solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)
        #train the model
        training_loss, validation_loss = minibatchSGDsolver.solve(train_data, train_labels, val_data, val_labels, model)
        #evaluate the accuracy
        curr_accuracy = model.accuracy(val_data, val_labels)
