...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                maxScore = None
                y = None
                fi = input_train_data[i]
                for label in self.legal_labels:
                    currScore = fi * self.weights[label]
                    if maxScore == None:
                        maxScore = currScore
                        y = label
                ...
                if y_star != y:
                    self.weights[y_star] = self.weights[y_star] + fi
                    self.weights[y] = self.weights[y] - fi
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        best100Features.append(self.weights[label])
