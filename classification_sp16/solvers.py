...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
	    ...
	    for i in range(len(param_vars)):
	       updated_tensor = param_vars[i] - self.learning_rate * grad_tensors[i]
	       updates.append((param_vars[i], updated_tensor))
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
	    for i in range(len(param_vars)):
            updated_var = self.momentum * vel_vars[i] - self.learning_rate * grad_tensors[i]
	        updated_tensor = param_vars[i] + updated_var
	        updates.append((param_vars[i], updated_tensor))
	        updates.append((vel_vars[i], updated_var))	
        #util.raiseNotDefined()
        return updates
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            t, v= train_data_generator.next(), val_data_generator.next()
	        train_loss = session.run([loss_tensor] + update_ops, feed_dict={placeholders[0]: t[0], placeholders[1]: t[1]})[0]
	        val_loss = session.run([loss_tensor], feed_dict={placeholders[0]: v[0], placeholders[1]: v[1]})[0]
